...
dir.create("shapefiles/invasion_data", recursive = TRUE, showWarnings = FALSE)
...
plot(st_geometry(Can_US_albers), col = "lightgrey", border = "black", main = "US + Canada Map")
...
Can_US_albers <- Can_US_albers %>%
  mutate( FIPS = ifelse(!is.na(full_fips), full_fips, FIPS))

Can_US_albers <- Can_US_albers %>%
  dplyr::select(-full_fips)


invaded_counties_all <- Can_US_albers%>%
  dplyr::filter(!is.na(BLD_Year)) %>%
  dplyr::select (FIPS, STATE_NAME, County, BLD_Year, geometry)
...
for(i in 2013:2014){
    ...
    invaded_in_i <- invaded_counties_all[which(invaded_counties_all$YrInv %in% i),]
    
    prev_invaded_counties <- invaded_counties_all[which(invaded_counties_all$YrInv %in% (2013:(i-1))),]
...
fig_DR <- ggplot(data=spread_df, aes(x=YrInv, y=dist_from_disc)) +
  ylab("Distance (km)")+
  xlab("Year")+ theme_bw()+
  geom_point(aes(x = YrInv, y = dist_from_disc), col=DR_col, size=2)+
...
col_bearings <- ggplot(sprd.rads.lwd, aes(x = bearing, y = spread_increment, fill=factor(year))) +
  ...
  scale_y_continuous(breaks = seq(0, 1500, 300), limits = c(0,1510), expand=c(0,0))+
  theme_bw() +
...
#tiff("fig1.tiff", units="in", width=6.85, height=8, res=900)
#ggarrange(invaded_area_discrete,wedges_p, ncol=1, nrow=2,
#          labels="auto", label.x = 0.13, label.y =1)
#dev.off()
#'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
ERR_col <- wes_palette("GrandBudapest1", 3, type="discrete")[3]
...
#---
resize.win(6.85,6)
ggarrange(bearing5_histogram,  fig_ERR_hist,
          fig_DR_hist, fig_BD_hist,
...
fig_ERR_precip_aniso <- ggplot(data=final_aniso, aes(x=MAP.mu.m, y=ERR_spread_rate_mu)) +
  ...
  scale_x_continuous(breaks = seq(1,1.4,0.1), limits=c(1,1.45), expand = c(0,0)) +
  scale_y_continuous(breaks = seq(0,60,10), limits=c(0,60), expand = c(0,0)) +
...
ggarrange(fig_ERR_red_aniso, fig_ERR_temp_aniso, fig_ERR_precip_aniso, ncol = 3, nrow = 1,
          labels="auto", label.x = 0.25, align="hv")
#'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
...
# County size through time and by wedge --------------------------------------------
#
#
#---
...
wedges_cty_area <- st_read("shapefiles/wedges/wedge_cty_area_sp.shp")
wedges_cty_area_df <- as.data.frame(wedges_cty_area)
wedges_cty_area_df$county_area_km <- as.numeric(wedges_cty_area_df$cty_a_km)
summary(wedges_cty_area_df$county_area_km)
summary(county_area_time$county_area_km)

ggplot(wedges_cty_area_df, aes(x=factor(FID), y=county_area_km)) + 
  geom_boxplot()+
  xlab('Wedge')+
  ylab(bquote('County area '(km^2)))+
  theme_classic() 
